...
def connect_to_google(service: str) -> Iterator[discovery.Resource]:
    ...
    credentials = service_account.Credentials.from_service_account_info(
        settings.GOOGLE_API_CREDENTIALS, scopes=scopes
    )
    api_version = {
        "calendar": "v3",
        "sheets": "v4",
...
def get_vacations(
    ...
    with connect_to_google("calendar") as conn:
        calendars = [
            item["id"]
            for item in conn.calendarList().list(fields="items(id)").execute()["items"]
        ]
        vacations = []
            ...
            for event in events["items"]:
                try:
                    ...
                    if {"start", "end"} <= event.keys():
                        ...
                        event["end"]["date"] = (
                            parse(event["end"]["date"]) - timedelta(days=1)
                        ).strftime(settings.JIRA_API_DATE_FORMAT)
                        user = search.get("user")
                        event["user"] = user
                        event["seconds"] = (
                            int(search.get("hours", 0) or 0) * SECONDS_IN_HOUR
                        )
                        vacations.append(event)
                except (
                    AttributeError,
                    TypeError,
                ):  # The event doesn't relate to vacations.
                    pass
