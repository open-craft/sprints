    ...
    def boards(name=""):
        ...
        if name:
            ...
            return [
                board
                for board in boards
                if board.name.startswith(settings.JIRA_SPRINT_BOARD_PREFIX)
            ]
        return boards
...
@pytest.mark.parametrize(
    ...
    [
        (
            "name=Sprint 197 (2019-06-18),startDate=2019-06-17T17:21:26.945Z",
            "Sprint 197 (2019-06-18)",
        ),
        (
            "name=TS.197 (2019-06-18),startDate=2019-06-17T17:21:26.945Z",
            "TS.197 (2019-06-18)",
        ),
    ],
)
...
@override_settings(
    ...
    SPILLOVER_REQUIRED_FIELDS=("Story Points", "Original Estimate"),
)
def test_prepare_spillover_rows():
    test_issues = [
        MockItem(
            key="TEST-1", fields=MockItem(story_points=1.0, original_estimate=7200)
        ),
        MockItem(
            key="TEST-2",
            fields=MockItem(
                story_points=3.14, original_estimate=9849
            ),  # estimated time should be rounded up here
        ),
    ]
    issue_fields = {
...
@override_settings(
    DEBUG=True
)  # `sentry_sdk` does not capture exceptions in `DEBUG` mode.
@pytest.mark.parametrize(
    "hours, expected",
    [
        ("12pm-9pm*", 0),
...
def test_get_sprint_meeting_day_division_for_member(hours, expected):
    ...
    assert _get_sprint_meeting_day_division_for_member(
        hours, sprint_start
...
def test_get_cell_member_roles_corrupted(mock_get):
    with open(
        "sprints/dashboard/tests/data/handbook/dummy_cells_corrupted.html", "r"
    ) as f:
        mock_get.return_value = Mock(text=f.read())
...
def test_get_rotations_roles_for_member():
    ...
    output = get_rotations_roles_for_member(
        "John Doe", {"FF": ["Jake Doe", "John Doe"], "DD": ["Jane Doe", "James Doe"]}
    )
    assert len(output) == 1
    ...
    output = get_rotations_roles_for_member(
        "John Doe", {"FF": ["Jake Doe", "James Doe"], "DD": ["Jane Doe", "James Doe"]}
    )
    assert len(output) == 0

    # When only partial name of the member is defined
    output = get_rotations_roles_for_member(
        "John Doe", {"FF": ["John Doe", "Jake Doe"], "DD": ["Jane Doe", "James Doe"]}
    )
    assert len(output) == 1
...
def test_compile_participants_roles():
    ...
    output = compile_participants_roles(
        members_data_dummy, rotations_data_dummy, roles_data_dummy
    )
